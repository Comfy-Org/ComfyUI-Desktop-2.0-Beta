...
export async function handleDelete({ event, installationId, inst }: ActionContext): Promise<ActionResult> {
  ...
  if (!markerContent) {
    return { ok: false, message: 'Safety check failed: this directory was not created by ComfyUI Desktop. Use Forget to remove it from the list, then delete the files manually.' }
  }
