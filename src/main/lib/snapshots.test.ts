...
describe('validateExportEnvelope', () => {
  ...
  it('rejects wrong type field', () => {
    expect(() => validateExportEnvelope({ ...makeEnvelope(), type: 'wrong' })).toThrow('not a ComfyUI Desktop snapshot export')
  })

  it('rejects missing type field', () => {
    const env = makeEnvelope()
    const { type: _, ...rest } = env
    expect(() => validateExportEnvelope(rest)).toThrow('not a ComfyUI Desktop snapshot export')
  })
