/**
 * Default preload for the host window's main webContents.
 *
 * The `buildElectronApi` builder is intentionally inlined here rather
 * than imported from a shared `./api` module: Electron 40 defaults to
 * `sandbox: true` for preloads, and sandboxed preloads can only
 * `require()` from a small whitelist (`electron`, `events`, `timers`,
 * `url`). Splitting api into a Rollup chunk produced a
 * `require("./chunks/api-*.js")` call that fails silently in the
 * sandbox, leaving `window.api` undefined and the renderer blank.
 *
 * Each preload entry is a single self-contained file; the duplication
 * with `comfyTitleBarPreload.ts` is the price of keeping every
 * preload sandbox-safe.
 */
