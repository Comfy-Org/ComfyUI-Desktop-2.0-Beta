/**
 * Builds the `window.api` bridge object exposed to all renderer surfaces.
 *
 * Extracted from `preload/index.ts` so the specialized title-bar /
 * title-menu preloads can mount the same bridge alongside their own
 * narrow surfaces (`window.__comfyTitleBar`, `window.__comfyTitleMenu`).
 * Without `window.api` in those renderers, `initializeRendererBootstrap()`
 * — which the title-bar and title-menu now call so steady-state ComfyUI
 * sessions emit telemetry — would crash on missing IPC methods.
 *
 * Telemetry must work no matter which renderer the user is interacting
 * with; sharing the api builder here is the smallest change that gets
 * `window.api` into every renderer without duplicating the bridge.
 */
