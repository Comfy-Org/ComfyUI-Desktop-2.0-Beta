...
interface TitleBarIdentityApi {
  ...
  isFirstUseTakeover: ComputedRef<boolean>
...
export function useTitleBarIdentity(opts: UseTitleBarIdentityOpts): TitleBarIdentityApi {
  ...
  /** Any active first-use takeover step (consent + post-consent). The
   *  title bar strips itself down to a minimal identity bar for the
   *  entire onboarding flow — waffle, downloads tray, and feedback all
   *  hide so the takeover screens read as a clean identity bar. Reverts
   *  to the full steady-state chrome once `firstUseMode === 'none'`. */
  const isFirstUseTakeover = computed(() => firstUseMode.value !== 'none')
