...
export interface FirstUseChainHooks {
  ...
  /** Drained by `openFlowTakeover` for the new-install branch — when
   *  set, the value is forwarded to `newInstallRef.value?.open({
   *  initialName })` so the form pre-seeds with the name the user
   *  picked on the takeover's `nameInstall` sub-step. The hook is
   *  responsible for clearing the ref after read so a follow-up
   *  non-chained new-install launch starts blank. */
  consumePendingFirstUseInstName: () => string | null
}
...
export function usePanelOverlays(opts: UsePanelOverlaysOpts): UsePanelOverlaysApi {
  ...
  async function openFlowTakeover(component: FlowComponent, entrypoint: string): Promise<void> {
    ...
    if (component === 'new-install') {
      // First-use chain may have stashed a pre-typed install name on
      // the takeover's `nameInstall` step (legacy-detected fork only);
      // forward it as `initialName` so the form pre-seeds. Cleared on
      // read inside the hook so a follow-up non-chained launch starts
      // blank.
      const initialName = opts.firstUseChain?.consumePendingFirstUseInstName() ?? null
