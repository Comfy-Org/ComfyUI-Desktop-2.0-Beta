...
async function pause(url: string): Promise<void> {
  await window.api.pauseModelDownload(url)
}

async function resume(url: string): Promise<void> {
  await window.api.resumeModelDownload(url)
}

async function cancel(url: string): Promise<void> {
  await window.api.cancelModelDownload(url)
}

async function showInFolder(savePath: string): Promise<void> {
  await window.api.showDownloadInFolder(savePath)
}
...
function clearCompleted(): void {
  for (const d of downloadStore.finishedDownloads) {
    if (d.status === 'completed') downloadStore.dismiss(d.url)
  }
}
</script>
  ...
  <div class="view active">
    <div class="toolbar">
      <div class="breadcrumb">
        <span class="breadcrumb-current">{{ t('downloads.title') }}</span>
      </div>
      <button
        v-if="downloadStore.finishedDownloads.some((d) => d.status === 'completed')"
