...
; nsDialogs gives us ${NSD_OnClick} for attaching a click handler to the
; Cancel button on the Finish page (MUI2 enables it visually via
; MUI_FINISHPAGE_CANCEL_ENABLED but doesn't wire a click action).
...
!macro customFinishPage
  ...
  # Wire a click handler onto the Cancel button (control ID 2 in NSIS's
  # wizard parent window). MUI2 with MUI_FINISHPAGE_CANCEL_ENABLED makes
  # the button visible/enabled but doesn't attach an action — without
  # this hook, clicking it is a no-op. ${NSD_OnClick} from nsDialogs
  # attaches OnFinishCancelClick (declared in customHeader) which
  # Quits cleanly. Same hook re-enables the title-bar X system-menu
  # item that SpiderBanner + the un-destroyed ShutdownBlockReason
  # leave greyed.
  Function FinishPageShow
    GetDlgItem $0 $HWNDPARENT 2
    ${NSD_OnClick} $0 OnFinishCancelClick
