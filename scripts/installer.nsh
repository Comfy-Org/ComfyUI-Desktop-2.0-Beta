...
; If the VC++ prerequisite can't be installed (user declines the UAC prompt),
; installVcRedist calls Abort. These give the Install page a clear failure
; header + the standard "Finish" button to close — instead of a frozen progress
; bar (electron-builder sets ShowInstDetails nevershow, so the abort log is
; hidden). Defined at top level so they apply before MUI_PAGE_INSTFILES.
!define MUI_INSTFILESPAGE_ABORTHEADER_TEXT "Installation failed"
!define MUI_INSTFILESPAGE_ABORTHEADER_SUBTEXT "The Microsoft Visual C++ Redistributable could not be installed. Please restart the installer and allow the permission prompt to continue."
...
!macro installVcRedist
  ...
  ${If} $2 == 2
    ...
    ${If} ${Errors}
      ...
      # Cancel on a hard prerequisite: fail the install cleanly. Abort (not Quit)
      # so MUI moves to its failure flow — the Install page shows the
      # MUI_INSTFILESPAGE_ABORTHEADER text ("Installation failed …") and the
      # button becomes Finish to close the wizard, like a standard installer.
      # SetErrorLevel gives a non-zero exit code for scripted/silent installs.
      SetErrorLevel 2
      Abort
    ${EndIf}
  ${Else}
