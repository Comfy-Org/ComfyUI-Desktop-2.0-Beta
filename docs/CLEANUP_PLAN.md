...
| `TitleBarApp.vue` | 🟡 **Split (in progress)** | Pull pill buttons + tooltip coordinator + update-pill state into composables. The tooltip composable will pair with **P1**. Done so far: `useTitleBarTooltip`, `useUpdatePills`, `useTitleBarMenus` (file-menu + downloads-tray + menu-reopen-guard map + isMenuOpen + downloadsState), `useTitleBarIdentity` (installLabel + sourceCategory + theme + fullscreen + first-use mode + install-type icon computeds + isLight). 1090 → 730 lines. Remaining shell carries: hover-gate state machine, install-less / activePanel refs, send-feedback handler, the wiring between the composables, and the template. |
