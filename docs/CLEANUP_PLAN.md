...
Initial sweep status (Thread F): scanning the new modules in `src/main/host/*`, `src/main/popups/*`, the renderer composables, and `src/main/lib/ipc/registerDownloadHandlers.ts` for AGENTS.md violations turned up no historical-narration comments (no "previously / now does / used to / formerly / extracted from") and a single `P0.9` plan reference (in `host/detach.ts`, fixed). The remaining long doc-blocks are genuine "why" notes (registry late-binding rationale, popup lifecycle invariants, IPC ordering) that AGENTS.md tells us to keep. No further trimming required for these modules.
