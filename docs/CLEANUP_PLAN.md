...
- ✅ **`createHostWindow` is monolithic** — split into three module-private helpers in [`host/createHostWindow.ts`](../src/main/host/createHostWindow.ts): `buildHostSkeleton(opts)` (BrowserWindow + titleBarView + comfyView + bound `layoutViews` + resize / move / fullscreen / focus listeners), `wireTitleBarHandshake(args)` (the `comfy-window:title-bar-ready` IPC handler that pushes initial state and pre-warms the file-menu / system-modal popups; returns the bound handler so the lifecycle module can `ipcMain.off` it on `closed`), and `installLifecycleListeners(args)` (the `'close'` consult-and-destroy flow + the `'closed'` registry/claim cleanup). `createHostWindow` is now a 35-line composition of the three helpers + the `entry` literal + `registerHostEntry`. A future `swapInstallInPlace` can re-run `wireTitleBarHandshake` + `installLifecycleListeners` against an existing skeleton without rebuilding the BrowserWindow.
- ✅ **`attachInstall` is monolithic** — extracted three independent `wire*` helpers in [`host/attach.ts`](../src/main/host/attach.ts): `wireKeyboardShortcuts({ entry, comfyContents, reloadComfy })` (F5 / Ctrl+R reload + Ctrl/Cmd zoom shortcuts → returns teardown), `wireFailRetry({ entry, comfyContents, comfyWindow, relaunchStates, currentComfyUrl })` (backoff retry on `did-fail-load` → returns `{ cancel }` which doubles as the relaunch-flow interrupt and the listener teardown), and `wireRenderProcessGone({ entry, comfyContents, reloadComfy })` (Datadog report + automatic reload → returns teardown). `attachInstall` itself shrank from ~330 to ~220 lines; remaining inline blocks (theme observer, OS title sync, installation-rename listener, dom-ready content script) share closure state (`currentInstallName` / `currentPageTitle` / `applyComfyTheme`) and are deferred to the seamless-transition feature, which will need to redesign that shared state to support an in-place install swap.
- ✅ **`computeBodyMode` is the right abstraction** — added `isChooserHost(entry)` / `isInstallHost(entry)` type-guard predicates next to it in [`host/registry.ts`](../src/main/host/registry.ts) and routed every semantic chooser-vs-install branch through them: `popups/titlePopup.ts` (file-menu builder + `new-install` / `track` / `load-snapshot` / `quick-install` activate guard), `host/createHostWindow.ts` (`will-prevent-unload`, `applyChooserHostTheme`, `applyChooserHostThemeToAll`), `host/attach.ts` (already-attached guard), `host/detach.ts` (`returnToDashboard` + `_detachInstallImpl` guards), `index.ts` (`refreshComfyTabBody` loop, chooser-pick claim guard, `transfer-host-bounds-to-install`). Registry-internal sites (the `installationIdToWindowKey` index in `registerHostEntry` / `unregisterHostEntry`, `findPreferredHostByVisibility` predicates inside the registry) keep direct `installationId` comparisons since they ARE the implementation the predicate is built from.
- ✅ **`pendingAttachClaims` is opaque** — encapsulated in [`host/registry.ts`](../src/main/host/registry.ts) behind `claimAttachHost(installationId, windowKey)`, `consumeAttachClaim(installationId)`, and `dropAttachClaimsForWindow(windowKey)`. The map is now module-private; the consumer in `onLaunch` and the cleanup in the host-window `'closed'` handler use the helpers instead of touching the Map directly. Tests in `registry.test.ts` cover the take-once contract and per-window cleanup.
...
| File | Action | Notes |
|---|---|---|
| `PanelApp.vue` | ✅ **Done** | Per-panel routing → composables. Done: `useAppUpdatePrompts`, `useSendFeedback`, `useDeepLinkRouter`, [`usePanelOverlays`](../src/renderer/src/panel/usePanelOverlays.ts) (overlay slot + Tier 2/3 helpers — `handleShowProgress`, `openFlowTakeover`, `openFirstUseTakeover`, `dismissTakeoverDirect`, `switchPanel`), [`useChooserHandoff`](../src/renderer/src/panel/useChooserHandoff.ts) (install-less chooser-tile launch + in-place attach claim + close-on-instance-started fallback), [`useFirstUseChain`](../src/renderer/src/panel/useFirstUseChain.ts) (cloud / chain-local / chain-migrate handlers + auto-launch watcher). PanelApp.vue 1203 → 456 lines. The three composables form a small dependency cycle (each consumes helpers from the others); broken with deferred lazy callbacks at construction time. |
| `TitleBarApp.vue` | ✅ **Done** | Pulled pill buttons + tooltip coordinator + update-pill state + hover gate into composables: `useTitleBarTooltip`, `useUpdatePills`, `useTitleBarMenus` (file-menu + downloads-tray + menu-reopen-guard map + isMenuOpen + downloadsState), `useTitleBarIdentity` (installLabel + sourceCategory + theme + fullscreen + first-use mode + install-type icon computeds + isLight), [`useTitleBarHoverGate`](../src/renderer/src/comfyTitleBar/useTitleBarHoverGate.ts) (`isHoverActive` + window-blur / pointermove / pointerleave listeners — drives `:hover` gating across menu, nav, and pill buttons; covered by [`e2e/title-bar-hover-gate.test.ts`](../e2e/title-bar-hover-gate.test.ts) for the install-less host and [`e2e/title-bar-hover-gate-comfy-window.test.ts`](../e2e/title-bar-hover-gate-comfy-window.test.ts) for the install-backed comfy-window host — past title-bar bugs have surfaced only after an install attached). 1090 → 674 lines. Remaining content is genuinely shell: ~100 lines of inline `Bridge` interface (typed IPC contract that mirrors `comfyTitleBarPreload.ts`), ~150 lines of template, ~270 lines of scoped CSS, and ~50 lines of composable wiring + the install-less / activePanel refs + send-feedback handler. |
| `SnapshotTab.vue` | ✅ **Done** | List / inspector are split. Inspector body (diff toggle + environment grid + custom-node + pip-package lists) extracted to [`SnapshotInspector.vue`](../src/renderer/src/components/SnapshotInspector.vue). Pure presentation helpers (`triggerClass`, `formatRelative`, `copyReasonLabel`, `changeSummary`, `diffHasChanges`) promoted to [`lib/snapshots.ts`](../src/renderer/src/lib/snapshots.ts) so the parent and inspector share one source. SnapshotTab.vue 1011 → 694 lines. |
| `ChooserView.vue` | ✅ **Done** | Per-install tile rendering (icon, kebab/error cluster, progress block, pills, close-instance CTA) extracted to [`chooser/ChooserInstallTile.vue`](../src/renderer/src/views/chooser/ChooserInstallTile.vue). The `chooser-tile-*` CSS lives in [`chooser/chooser-tiles.css`](../src/renderer/src/views/chooser/chooser-tiles.css) and is imported from both component scoped style blocks so the New Install / Cloud entry tiles and per-install tiles render identically without duplicating the rules. ChooserView.vue 924 → 306 lines. |
...
| `lib/ipc/shared.ts` | ✅ **Done** | Coherent — left intact (no split). Trimmed three doc-blocks per AGENTS.md "one sentence beats five" while keeping the substance: `onThemeChanged` callback (6 → 4 lines), the `_addSession` per-category recency stamp (7 → 3 lines), and the `_resolveAndBroadcastVersions` baseTag-clobber guard (17 → 9 lines). Other multi-line comments in the file (`_extraBroadcastTargets` API-quirk note, `getAppVersion` git-describe gating) are genuine "why" notes that AGENTS.md tells us to keep. 637 → 623 lines. |
...
Sweep status (Thread F): scanning the new modules in `src/main/host/*`, `src/main/popups/*`, the renderer composables (`useAppUpdatePrompts`, `useSendFeedback`, `useDeepLinkRouter`, `useTitleBarMenus`, `useTitleBarIdentity`, `useTitleBarTooltip`, `useUpdatePills`, `useTitleBarHoverGate`, `usePanelOverlays`, `useChooserHandoff`, `useFirstUseChain`), and `src/main/lib/ipc/registerDownloadHandlers.ts` for AGENTS.md violations turned up no historical-narration comments and a single `P0.9` plan reference (in `host/detach.ts`, fixed). The remaining long doc-blocks are genuine "why" notes (registry late-binding rationale, popup lifecycle invariants, IPC ordering, overlay tier-collision rules, first-use chain auto-launch invariant) that AGENTS.md tells us to keep. No further trimming required for these modules.
